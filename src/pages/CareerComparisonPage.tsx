...
export default function CareerComparisonPage() {
  ...
  useEffect(() => {
    if (selectedA && selectedB) {
      setLoading(true);
      fetch(`/api/compare-careers?ids=${selectedA},${selectedB}`)
        .then(res => res.json())
        .then(data => {
      ...
      if (!selectedA) setCareerA(null);
      if (!selectedB) setCareerB(null);
    }
  }, [selectedA, selectedB]);
  ...
  const ComparisonRow = ({ 
    ...
    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_1fr] border-b border-stone-100 last:border-0">
      ...
      <div className="p-6 md:border-r border-stone-100 bg-white">
        {isList ? (
  ...
  return (
    <div className="space-y-12 py-12 max-w-7xl mx-auto px-4">
      <header className="text-center space-y-6 max-w-3xl mx-auto">
        ...
        <h1 className="text-5xl font-bold tracking-tight text-stone-900">Career Comparison Tool</h1>
        <p className="text-stone-600 text-xl leading-relaxed">
          Select two careers to compare them side-by-side. Understand the differences in responsibilities, 
          skills, and future outlook to make an informed choice.
        </p>
      </header>
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <span className={selectedA ? 'text-stone-900 font-medium' : 'text-stone-400'}>
                  {allCareers.find(c => c.id === selectedA)?.career_name || 'Select first career...'}
                </span>
              </div>
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <span className={selectedB ? 'text-stone-900 font-medium' : 'text-stone-400'}>
                  {allCareers.find(c => c.id === selectedB)?.career_name || 'Select second career...'}
                </span>
              </div>
      ...
      <div className="relative min-h-[400px]">
        ...
        {careerA && careerB ? (
          ...
          >
            {/* Table Header */}
            <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_1fr] bg-stone-900 text-white">
              ...
              <div className="p-8 text-center md:border-r border-stone-800">
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-stone-50 border-2 border-dashed border-stone-200 rounded-[2.5rem] text-stone-400 space-y-4">
            <ArrowLeftRight className="w-16 h-16 opacity-20" />
            <p className="text-xl font-medium">Select two careers above to start comparing</p>
          </div>
        )}
      </div>
    </div>
